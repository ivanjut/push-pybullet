...
output_mean = np.mean(np.absolute(outputs), axis=0)
...
for i in range(data_points):
    ...
    mlp_dist_error += abs(mlp.predict(X_test[i:i+1,:])[0][0] - y_test[i:i+1,:][0][0])
    mlp_offset_error += abs(mlp.predict(X_test[i:i+1,:])[0][1] - y_test[i:i+1,:][0][1])
    mlp_orn_error += abs(mlp.predict(X_test[i:i+1,:])[0][2] - y_test[i:i+1,:][0][2])
...
print("------------------------------------------------------------------")
print("Score: ", mlp.score(X_test, y_test, sample_weight=None))
...
for i in range(data_points):
    ...
    # print("Actual:     ", y[i:i+1,:])
    lr_dist_error += abs(lr.predict(X_test[i:i+1,:])[0][0] - y_test[i:i+1,:][0][0])
    lr_offset_error += abs(lr.predict(X_test[i:i+1,:])[0][1] - y_test[i:i+1,:][0][1])
    lr_orn_error += abs(lr.predict(X_test[i:i+1,:])[0][2] - y_test[i:i+1,:][0][2])
...
print("------------------------------------------------------------------")
print("Score: ", lr.score(X_test, y_test, sample_weight=None))